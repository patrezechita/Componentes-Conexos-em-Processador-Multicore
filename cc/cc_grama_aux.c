...
void DFS(grafo_t G[], int i, int vInicial, int vFinal, int vPai, int thID, int *);
...
// alocação dinamica da nahri, preciso revisar
	...
	#pragma omp parallel for private(thID, vPai, i, vInicial, vFinal, qtdStructArestaLocal) shared(G, infoThread)
	for(thID=0; thID<nThread; thID++)
	{
		...
		for(i=vInicial; i<=vFinal; i++)
		{
			...
			{
				vPai = i;
				DFS(G, i, vInicial, vFinal, vPai, thID, &qtdStructArestaLocal);
			}
		}
		qtdStructAresta[thID] = qtdStructArestaLocal;
	}
	...
	tEsq = 0;
	// para cada árvore gerada pelo DFS
	// printf("\nordem dos union-find\n");
	for(tDir=1; tDir<nThread; tDir++)
	{
		...
		for(p=0; p<qtdStructAresta[tDir]; p++)
		{
			...
			if(pID != qID)
			{
				...
				for(i=0; i<qtdVertices; i++) {
					if(resultadoDFS[tEsq][i] == pID || resultadoDFS[tEsq][i] == qID) {
						// da o minimo para o pai por motivos de padronizacao
						resultadoDFS[tEsq][i] = min;
					}
				}

				// atualiza  alista de arestas
				//printf("insere na t%d a aresta <%d;%d>\n", tEsq, u, v);
				aux = qtdStructAresta[tEsq];
				structAresta[tEsq][aux][0] = u;
				structAresta[tEsq][aux][1] = v;
				qtdStructAresta[tEsq]++;
			}
		}
	}
...
void DFS(grafo_t G[], int i, int tBegin, int tLast, int vPai, int thID, int *qtdStructArestaLocal)
{
	int u, v;
	...
	while (current != NULL)
	{
		...
		{
			structAresta[thID][(*qtdStructArestaLocal)][0] = u;
			structAresta[thID][(*qtdStructArestaLocal)][1] = v;
			(*qtdStructArestaLocal)++;
		}
		...
		if(v >= tBegin && v <= tLast)
		{
			...
			if(G[v].cor == 'B')
			{
				...
				G[v].pai = u;
				DFS(G, v, tBegin, tLast, vPai, thID, qtdStructArestaLocal);
			}
		}
